...
<xsl:stylesheet exclude-result-prefixes="xs" version="3.0"
    ...
    <xsl:template match="/">
        ...
        <!-- Keywords with information on quicktag hierarchy -->
        <xsl:result-document href="_output/keywords_quicktags.xml">
            <tss:keywords>
                <xsl:copy-of select="$v_tags"/>
            </tss:keywords>
        </xsl:result-document>
    </xsl:template>
...
<!--                    <xsl:attribute name="correspReference" select="concat('#uuid_', $p_reference-uuid)"/>-->
    ...
    <!-- template for generating a list of <tss:keywords> with their corresponding quicktag hierarchy -->
